...
#: Схема API. Лежит вне /api и поэтому попадала под правило «это интерфейс».
#: На своей машине она удобна, а на сервере в интернете это готовая карта
#: всех ручек для постороннего. При выключенном входе доступ остаётся
#: свободным: охрана в этом случае пропускает всех как хозяина.
