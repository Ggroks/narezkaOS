...
**Музыка — единственный тег со знаком минус.** Площадки узнают чужую музыку
в фоне и в ответ закрывают доступ к ролику или забирают доход с него, поэтому
её присутствие работает штрафом, а не признаком (§54).
"""
...
TAGS: dict[str, tuple[int, ...]] = {
    ...
    "crowd": (64,),
    # Музыка: единственный тег со знаком минус — из-за прав на неё.
    "music": (132,),
}
...
class TagTrack:
    ...
    def share(self, name: str, start: float, end: float, threshold: float = MIN_SCORE) -> float:
        ...
        музыка на протяжении всего ролика — это уже основание для претензии
        по правам.
        """
        track = self.scores.get(name)
        if track is None or track.size == 0:
