...
class FramingConfig(BaseModel):
    ...
    #: приближённый контент снизу (§61); track — узкий кадр, ведомый за лицом;
    #: pip — лицо врезкой поверх содержимого; camera — только вебка во весь
    #: кадр. Три последние применяются там, где стадия facecam нашла вебку
    #: наложением; иначе кадрируется как обычно и об этом говорится в журнале.
    layout: Literal["single", "split", "track", "pip", "camera"] = "single"
    ...
    #: Раскладка «сплит»: какую долю высоты занимает вебка сверху и как
    #: близко берётся лицо. Треть кадра и 2.6 ширины лица — то, что вышло
    #: по замерам на записи со стримером; в интерфейсе это ползунки.
    split_top_share: float = Field(default=0.34, ge=0.2, le=0.5)
    face_zoom: float = Field(default=2.6, ge=1.5, le=4.0)
    #: Где по высоте полосы оказывается центр лица. Чуть выше середины —
    #: тогда в кадр входят плечи, а не пустота над головой.
    face_vertical: float = Field(default=0.45, ge=0.1, le=0.9)
    #: Вести ли кадр за головой в раскладках «сплит» и «только вебка».
    #: Стример за клип уходит из статичной рамки: она берётся по одному
    #: кадру, а он двигается. Стоит времени детектора — поэтому выбором,
    #: а не всегда: траектория считается только когда она нужна.
    follow_face: bool = False
    #: Врезка: доля ширины кадра под окошко с лицом, отступ от края в долях
    #: этого окошка и угол. Врезка вспомогательная — по умолчанию треть
    #: ширины в левом верхнем углу: правый нижний площадки перекрывают
    #: кнопками, а левый верхний остаётся свободным.
    pip_share: float = Field(default=0.33, ge=0.15, le=0.6)
    pip_margin: float = Field(default=0.12, ge=0.0, le=1.0)
    pip_corner: Literal["top_left", "top_right", "bottom_left", "bottom_right"] = "top_left"
...
class SubtitlesConfig(BaseModel):
    #: Набор оформления по умолчанию. Для записи его меняют в интерфейсе,
    #: здесь — то, с чего начинают все новые. Старые имена (STYLE_1…)
    #: продолжают работать: записи, сделанные до пресетов, ссылаются на них.
    style: str = "classic"
