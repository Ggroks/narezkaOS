...
Ты подписываешь части длинной нарезки, чтобы получилось оглавление под видео.

Тебе дают куски одного занятия по порядку. Для каждого напиши короткое
название: что там происходит. Не пересказ, а указатель — по нему зритель
решает, стоит ли перематывать сюда.

Правила:
— 2-6 слов, без точки в конце;
— конкретно: «спор о накрутке ботов», а не «продолжение разговора»;
— не повторяй одно название дважды: если два куска про одно, отметь развитие
  («ещё о ботах: цифры»);
— пиши по-русски, обычными словами.

Отвечай ТОЛЬКО массивом JSON по числу кусков, без пояснений:
["название первого", "название второго", ...]
"""


def build_chapters_message(pieces: list[tuple[float, str]]) -> str:
    """Запрос на подписи к кускам нарезки."""
    blocks = []
    for number, (start, text) in enumerate(pieces, start=1):
        minutes, seconds = divmod(int(start), 60)
        blocks.append(f"Кусок {number} (с {minutes}:{seconds:02d} записи):\n{text[:1500]}")
    return "\n\n".join(blocks)


MARKS_SYSTEM_PROMPT = """\
Ты размечаешь оглавление готовой нарезки — списком таймкодов для описания.

Тебе дают расшифровку ролика с временем ОТ НАЧАЛА РОЛИКА. Отметь места,
где разговор переходит к новой теме, и назови каждую часть.

Правила:
— первая глава всегда начинается с 0;
— между главами не меньше минуты: оглавление из тридцати строк бесполезно;
— название 2-6 слов, конкретное: «спор о накрутке ботов», а не «продолжение»;
— на двадцать минут обычно выходит 5-10 глав, не больше;
— пиши по-русски, обычными словами.

Отвечай ТОЛЬКО массивом JSON, без пояснений:
[{"at": секунды_от_начала_ролика, "title": "название"}]
"""


def build_marks_message(digest: str, total: float) -> str:
    minutes = int(total // 60)
    return (
        f"Ролик длится {minutes} минут. Время в скобках — от начала ролика.\n\n{digest}"
    )
