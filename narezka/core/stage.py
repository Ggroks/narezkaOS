...
class Stage(ABC):
    ...
    #: К какому куску работы относится стадия: `analysis` — разбор записи
    #: (расшифровка и поиск моментов), `shorts` — сборка коротких роликов,
    #: `long` — длинная нарезка. Человек запускает работу по частям, а не
    #: пайплайн целиком: короткие ролики нужны не всем, длинная нарезка —
    #: тем более, а поиск эпизодов вдобавок стоит запросов к модели.
    #: Разбор нужен обеим сборкам, поэтому входит в любой запуск (§44).
    group: ClassVar[str] = "analysis"
